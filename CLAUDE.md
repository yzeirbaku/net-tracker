...
- **Inline SVG icons, not Unicode glyphs.** Every icon inside the icon-button family is an inline Lucide-style `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" …>` with one or more `<path>` / `<line>` children. `stroke="currentColor"` lets the danger hover-color rule re-tint automatically. CSS sizing is centralized: `.budget-icon-btn > svg` / `.ah-action-btn > svg` are 14px (danger ×) / 12px (everything else); the tick check bumps to 13px to balance its lighter two-stroke weight, the month-nav chevrons render at 18px in the bigger `.budget-nav` box, and the header burger at 22px desktop / 26px mobile. Reason: Unicode glyphs (`✎` `×` `✓` `↺` `‹` `›` `☰`) drift in size, baseline, and centering across iOS Safari vs Chrome — SVGs render pixel-identical.
