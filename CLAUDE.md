...
- **Custom UI components:**
  ...
  - `frontend/shared/ui.js::withBusyButton(btn, busyLabel, fn)` — wraps any backend-calling button so it disables + relabels (`"Adding…" / "Saving…" / "Deleting…"`) for the duration and restores on success or failure. **Use this around every button click that fires an API call** so the user can't double-submit.
  - `frontend/shared/ui.js::friendlyError(err, fallbackPrefix)` — maps the backend's `HTTPException.detail` code (e.g. `account_name_taken`, `before_earliest_entry`) into a user-facing message. Falls back to `"{fallbackPrefix}. Please try again."` for unknown codes. **Always use this when surfacing API errors** — never put `err.message` into a toast or error card directly.
...
- **Busy buttons.** Any button that fires a backend call or otherwise long-running process MUST switch to disabled + a verb-form busy label (`"Adding…"`, `"Saving…"`, `"Sending…"`, `"Deleting…"`) for the duration of the call and restore on completion. Use `withBusyButton()` from `shared/ui.js` — don't roll the disable/restore manually. The motivating bug: a double-tap on Add Account fired two POSTs and surfaced the duplicate-name error from the second one.
- **No backend leakage in user-facing errors.** The frontend MUST NEVER show raw backend codes, SQL constraint names, Python field names, HTTP methods/paths, or status codes to the user. Catch every API error through `friendlyError(err, fallbackPrefix)` so known `HTTPException.detail` codes get mapped to friendly sentences and unknown ones fall back to `"{prefix}. Please try again."`. To add a new error code: define it in `backend/app/<module>.py` as a snake_case detail string, then add the user-facing copy to `_ERROR_MESSAGES` in `shared/ui.js`. Generic 500s / network errors get the fallback prefix automatically.
- **No auto-focus ring on dialog open.** `<dialog>.showModal()` programmatically focuses the first focusable child, and iOS Safari (sometimes Firefox too) renders that as `:focus-visible` — painting a permanent-looking focus ring on Cancel / Close until the user taps elsewhere. **Every code path that calls `showModal()` MUST follow it with `blurAutoFocusedInDialog(dlg)` from `shared/ui.js`** (which schedules a `.blur()` on the next animation frame; the dialog's modal focus trap still works). `openDialog()` and `confirmPrompt()` already do this; bespoke open flows (anything that calls `dlg.showModal()` directly) need the helper too.
