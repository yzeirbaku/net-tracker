...
- **No auto-focus ring on dialog open.** `<dialog>.showModal()` auto-focuses the first focusable child, and on Chromium/Safari that paints the input border in accent green until the user clicks elsewhere. Every `showModal()` call must follow with `blurAutoFocusedInDialog(dlg)` from `shared/ui.js` — the helper blurs synchronously (same task as `showModal`, so no flash paints) and re-blurs on the next animation frame as an iOS Safari fallback. `openDialog()` and `confirmPrompt()` already do this; bespoke open flows that call `dlg.showModal()` directly need the helper too.
