...
- **Profile** — drawer label and view title are both "Profile" (internal routing key remains `data-action="settings"` and `view-settings` so app.js wiring is untouched; only user-facing strings changed). Layout top-to-bottom: signed-in email, Categories accordion, Accounts accordion, a `Download current state · zip` button, then a final card titled `Settings` holding the Theme toggle and the Current-month advance toggle. The download bundles every subsystem's CSV (template, every stamped month, put-aside, net worth) via the dependency-free ZIP writer at `frontend/shared/zip.js` (STORE method, ~150 lines); orchestration is `settings.js::downloadStateBundle()`. Per-subsystem CSV builders: `buildTemplateCsv` / `buildMonthCsv` (`budget-common.js`), `buildPutAsideCsv` (`put-aside.js`), `buildNetWorthCsv` (`networth.js`). Sign in/out lives in the drawer, not on this page.
