...
- **Totals footer.** Shared `.budget-footer` across month, template-editor, and version views: Total planned / Salary / Free money (month adds Spent so far). Free money is green when ≥ 0, red below. Template editor uses `updateTemplateFooter()` to refresh in place without losing the input caret.
- **Amount input formatting.** Every monetary input is wired through `installAmountFormatter()` (in `budget.js`): live dot-thousands as the user types, caret preserved. `parseAmount()` strips dots on submit; `formatAmountForInput()` renders initial values.
