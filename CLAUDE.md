...
- **Budget** — persistent template (draft + labelled snapshots) stamped into per-month plans with checkable items. Past months can't be stamped; archive locks a month read-only. Template editor supports CSV upload (rejects unknown category names; Save commits). Per-view CSV downloads were removed — the whole-app `Download Current Net` button on Profile covers it.
...
- **Profile** — drawer label and view title are both "Profile" (internal routing key remains `data-action="settings"` and `view-settings` so app.js wiring is untouched; only user-facing strings changed). Layout top-to-bottom: signed-in email, Categories accordion, Accounts accordion, a `Settings` card holding the Theme toggle and the Current-month advance toggle, and finally a `Download Current Net` button at the very bottom. The download is the *only* CSV export in the app — it bundles every subsystem (template, every active stamped month, put-aside, net worth) into one ZIP via the dependency-free writer at `frontend/shared/zip.js` (STORE method, ~150 lines). **Archived months are deliberately excluded** — once archived a month is frozen history, not part of "current net," so `downloadStateBundle()` filters `monthsRes` to `!archived_at` before fetching detail rows. Orchestration is `settings.js::downloadStateBundle()`; per-subsystem CSV builders are `buildTemplateCsv` / `buildMonthCsv` (`budget-common.js`), `buildPutAsideCsv` (`put-aside.js`), `buildNetWorthCsv` (`networth.js`). Sign in/out lives in the drawer, not on this page.
